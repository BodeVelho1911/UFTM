...
"""Programa que analisa os dados obtidos em um ensaio de tração
Trocar o caminho do arquivo dos dados (.csv) em 'file_path'."""
    
# Caminho para arquivo
file_path = "./Dados - Tração/aluminio_tracao.csv"
...
# Dados iniciais
L0 = 50 # Comprimento inicial do extensômetro [mm]
d = 7.778 # Diâmetro do CDP [mm]
...
ind = (df["Rel. Defor."] - 0.003).abs().idxmin() # Index final da fase elástica
...
# Linha paralela à fase elástica (2% transladada)
...
plt.title(label="Ensaio de tração - Alumínio")
...
print(f"Limite de Resistência à Tração: {Sut:.2f} MPa")
print(f"Módulo de Young: {E:.2f} GPa")
print(f"Limite de Escoamento: {sigma_e:.2f} MPa")
print(f"Resiliência: {res:.1f} J/m³")
print(f"Tenacidade: {ten:.1f} J/m³")